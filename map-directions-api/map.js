...
function sortCoordsByDistance(userLng, userLat) {
    ...
    const url = `https://api.mapbox.com/directions/v5/mapbox/walking/${userLng},${userLat};${coordonate.map(coord => coord.join(',')).join(';')}?alternatives=false&geometries=geojson&overview=simplified&steps=false&access_token=${mapboxgl.accessToken}`;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            sortedCoords.forEach((coord, index) => {
                const numeIzvor = Object.keys(izvoareCoordonate).find(key => izvoareCoordonate[key][0] === coord[0] && izvoareCoordonate[key][1] === coord[1]);
